...
- **Follow-up on the default branch (not this branch):** add `chess` to `matrix: branch: [sudoku, ledger]` in `.github/workflows/sync-upstream.yml` so this branch receives the weekly `main` merge like the other tool branches.
